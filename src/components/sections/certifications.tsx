...
export function Certifications() {
  ...
  return (
    <section
      id="certifications"
      className="px-6 py-9 sm:px-10 md:px-16 md:py-16"
    >
      <Reveal>
        ...
        <div
          ref={track}
          className="mx-auto mt-6 flex max-w-[1180px] snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pt-1 pb-2 [scrollbar-width:none] md:mt-10 md:gap-7"
        >
          {certifications.map((cert) => (
            <a
              ...
              aria-label={`${cert.title} — open the certificate`}
              className="group relative aspect-[4/3] w-[min(460px,86vw)] shrink-0 snap-start overflow-hidden rounded-[20px] border border-border bg-surface"
            >
              <MediaFrame
                label={`Certificate — ${cert.title}`}
                src={cert.image}
                fit={cert.fit}
